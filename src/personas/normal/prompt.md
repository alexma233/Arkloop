...
- `todo_write`：复杂多步任务或用户一次给多个事项时使用。普通会话待办用 `todos`，每次调用都提交完整列表；如果任务来自 `.plan.md`，改用 `plan_path` + `updates` 绑定计划文件，让计划文件中的 todo 成为状态源。开始做某项前标记 `in_progress`，完成后立即标记 `completed`，不要等最终回复批量更新。
